...
enum ApartmentApiProtocol: ServicesApiRouterProtocol {
    ...
    var parameters: Parameters? {
        ...
        case .createPost(_, let data):
            return [ "apartment_id" : data.apartment_id, "currency_id" : data.currency_id, "lat": data.lat, "lng" : data.lng, "address" : data.address, "name" :data.name, "street" : data.street, "content" : data.content, "ward_id" : data.ward_id, "total_area": data.total_area, "district_id" : data.district_id, "province_id" : data.province_id, "apartment_code" :data.apartment_code, "apartment_number" : data.apartment_number, "property_type_id" : data.property_type_id, "post_type_id" : data.post_type_id, "images": "", "area" : data.area, "area_unit_id" :data.area_unit_id, "bathroom_number" : data.bathroom_number, "bedroom_number" :data.bedroom_number, "contacts" : ["name": data.contacts.first!.name, "phone" :data.contacts.first!.phone, "email": data.contacts.first!.email], "description" : data.description, "direction" : data.direction, "end_date": data.end_date, "floor_number" : data.floor_number, "garden" : data.garden, "pool" :data.pool, "price" : data.price, "region" :data.region ?? "", "start_date": data.start_date, "rooftop" : data.rooftop
            ]
        default:
            return [:]
        }
